(function() {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    if (Array.isArray(collection)) {
      ...
      for (var i = 0; i < collectionCopy.length; i++) {
        accumulator = iterator(accumulator, collectionCopy[i]);
      } 
    } else {
      for (var key in collection) {
        if (accumulator === undefined) {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function (isTrue, item) {
      var booleanItem = Boolean(iterator(item));
      return booleanItem && isTrue;
    }, true);
  };
  ...
  _.memoize = function(func) {
    ...
    var callback = function () {
      ...
      var inputIdx = _.indexOf(calledValues, stringArgs);
      
      if (inputIdx !== -1) {
        return results[inputIdx];
      } else {
        calledValues.push(stringArgs);
  ...
  _.shuffle = function(array) {
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      var temp = shuffled[randomNumber];
